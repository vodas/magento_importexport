 ...
 */
class DB_Prodexport_Model_Io extends Mage_Dataflow_Model_Convert_Adapter_Abstract
{
    ...
    public function getResource($forWrite = false)
    {
        if (!$this->_resource) {
            ...
            $isError = false;
                    ...
                    asort($files);
                    $file = array_values($files)[1];

                    $this->setVar('filename',$file);
                    $this->setVar('path',$myPath);
                    $ioConfig['path'] = $myPath;
                    $ioConfig['filename'] = $file;
    ...
    public function load()
    {
        ...
        copy($filename, Mage::getBaseDir()."/var/import/regular/processed/".$ioConfig['filename']);
        unlink($filename);
        return $this;
    }
